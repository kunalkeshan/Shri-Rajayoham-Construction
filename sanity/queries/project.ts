...
const PROJECT = {
	...
	getIndividual: groq`*[_type == "project" && slug.current == $slug][0]{ ..., "slug": slug.current, "image": {"url": coverImage.asset->url, "alt": coverImage.alt}, services[]->, packages[]->{..., services[]->, "slug": slug.current, "image": {"url": image.asset->url, "alt": image.alt}}, imageGallery[]{..., "image": {"url": image.asset->url, "alt": image.alt}}, testimonials[]->{..., "image": {"url": image.asset->url, "alt": image.alt}}, teamMembers[]->{..., "image": {"url": image.asset->url, "alt": image.alt}, "slug": slug.current} }`,
};
