		...
		<div className='w-full h-full overflow-x-hidden'>
			...
			>
				<CarouselContent className=''>
					{data.map((item, index) => (
						<HeroCarouselItemWithImage
							item={item}
							key={`hero-coursel-item-${index}`}
						/>
					))}
				</CarouselContent>
...
interface HeroCarouselItemWithImageProps {
	item: (typeof data)[number];
}

const HeroCarouselItemWithImage: React.FC<HeroCarouselItemWithImageProps> = ({
	item,
	...props
}) => {
	const loaded = useProgressiveImage(item.image);
	return (
		<CarouselItem {...props}>
			<div className=''>
				<div
